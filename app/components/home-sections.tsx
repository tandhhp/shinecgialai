...
export function ConsultationForm() {
  ...
  return (
    <div className="consultation-modal" role="dialog" aria-modal="true" aria-labelledby="consultation-title" onMouseDown={() => setIsConsultationFormOpen(true)}><div className="consultation-card" onMouseDown={(event) => event.stopPropagation()}>
      {isSubmitted ? <div className="form-success"><i>✓</i><p className="eyebrow green">ĐĂNG KÝ THÀNH CÔNG</p><h2>Cảm ơn quý nhà đầu tư</h2><p>Thông tin đã được tiếp nhận. Đội ngũ Shinec Gia Lai sẽ liên hệ trong thời gian sớm nhất.</p><button className="button lime" onClick={() => setIsConsultationFormOpen(false)}>ĐÓNG <b>→</b></button></div> : <><button className="close-form" onClick={() => setIsConsultationFormOpen(false)} aria-label="Đóng biểu mẫu">×</button><p className="eyebrow green">SHINEC GIA LAI</p><h2 id="consultation-title">ĐĂNG KÝ NHẬN TƯ VẤN</h2><p className="form-intro">Để lại thông tin, chúng tôi sẽ liên hệ và gửi hồ sơ đầu tư phù hợp với nhu cầu của quý doanh nghiệp.</p><form onSubmit={() => setIsSubmitted(true)}> <label>HỌ VÀ TÊN<input name="fullName" required placeholder="Nhập họ và tên" /></label><label>SỐ ĐIỆN THOẠI<input name="phone" required type="tel" pattern="[0-9+ ]{8,}" placeholder="Nhập số điện thoại" /></label><label>EMAIL<input name="email" required type="email" placeholder="Nhập địa chỉ email" /></label><label>NHU CẦU QUAN TÂM<select name="interest" defaultValue=""><option value="" disabled>Chọn nhu cầu tư vấn</option><option>Thuê đất công nghiệp</option><option>Hạ tầng và tiện ích</option><option>Nhận hồ sơ đầu tư</option><option>Khác</option></select></label><label>GHI CHÚ<textarea name="message" rows={3} placeholder="Nội dung cần tư vấn" /></label><label className="consent"><input required type="checkbox" /> <span>Tôi đồng ý để Shinec Gia Lai liên hệ và sử dụng thông tin phục vụ tư vấn đầu tư.</span></label><button className="button lime" type="submit">GỬI ĐĂNG KÝ <b>→</b></button></form></>}</div></div>
  )
}
